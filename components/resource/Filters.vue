...
const formats = [
  { value: 'course', label: 'Courses', icon: 'M4.26 10.147a60.436 60.436 0 00-.491 6.347A48.627 48.627 0 0112 20.904a48.627 48.627 0 018.232-4.41 60.46 60.46 0 00-.491-6.347m-15.482 0a50.57 50.57 0 00-2.658-.813A59.905 59.905 0 0112 3.493a59.902 59.902 0 0110.399 5.84c-.896.248-1.783.52-2.658.814m-15.482 0A50.697 50.697 0 0112 13.489a50.702 50.702 0 017.74-3.342M6.75 15a.75.75 0 100-1.5.75.75 0 000 1.5zm0 0v-3.675A55.378 55.378 0 0112 8.443m-7.007 11.55A5.981 5.981 0 006.75 15.75v-1.5' },
  { value: 'book', label: 'Books', icon: 'M12 6.042A8.967 8.967 0 006 3.75c-1.052 0-2.062.18-3 .512v14.25A8.987 8.987 0 016 18c2.305 0 4.408.867 6 2.292m0-14.25a8.966 8.966 0 016-2.292c1.052 0 2.062.18 3 .512v14.25A8.987 8.987 0 0018 18a8.967 8.967 0 00-6 2.292m0-14.25v14.25' },
  { value: 'tool', label: 'Tools', icon: 'M11.42 15.17L17.25 21A2.652 2.652 0 0021 17.25l-5.877-5.877M11.42 15.17l2.496-3.03c.317-.384.74-.626 1.208-.766M11.42 15.17l-4.655 5.653a2.548 2.548 0 11-3.586-3.586l6.837-5.63m5.108-.233c.55-.164 1.163-.188 1.743-.14a4.5 4.5 0 004.486-6.336l-3.276 3.277a3.004 3.004 0 01-2.25-2.25l3.276-3.276a4.5 4.5 0 00-6.336 4.486c.091 1.076-.071 2.264-.904 2.95l-.102.085m-1.745 1.437L5.909 7.5H4.5L2.25 3.75l1.5-1.5L7.5 4.5v1.409l4.26 4.26m-1.745 1.437l1.745-1.437m6.615 8.206L15.75 15.75M4.867 19.125h.008v.008h-.008v-.008z' },
  { value: 'podcast', label: 'Podcasts', icon: 'M12 18.75a6 6 0 006-6v-1.5m-6 7.5a6 6 0 01-6-6v-1.5m6 7.5v3.75m-3.75 0h7.5M12 15.75a3 3 0 01-3-3V4.5a3 3 0 116 0v8.25a3 3 0 01-3 3z' },
  ...
  { value: 'article', label: 'Articles', icon: 'M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m0 12.75h7.5m-7.5 3H12M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9z' },
  { value: 'video', label: 'YouTube', icon: 'M15.75 10.5l4.72-4.72a.75.75 0 011.28.53v11.38a.75.75 0 01-1.28.53l-4.72-4.72M4.5 18.75h9a2.25 2.25 0 002.25-2.25v-9a2.25 2.25 0 00-2.25-2.25h-9A2.25 2.25 0 002.25 7.5v9a2.25 2.25 0 002.25 2.25z' },
...
function toggleTime(time: string) {
  filters.value = { ...filters.value, estimatedTime: toggleArrayValue(filters.value.estimatedTime, time) }
}
...
function toggleFeatured() {
  filters.value = { ...filters.value, featured: filters.value.featured ? undefined : true }
}

function toggleHiddenGem() {
  filters.value = { ...filters.value, hiddenGem: filters.value.hiddenGem ? undefined : true }
}

function toggleBreakthrough() {
  filters.value = { ...filters.value, breakthrough: filters.value.breakthrough ? undefined : true }
}
...
const hasActiveFilters = computed(() => {
  return (filters.value.topics?.length ?? 0) > 0 ||
    ...
    (filters.value.estimatedTime?.length ?? 0) > 0 ||
    !!filters.value.category ||
    !!filters.value.featured ||
    !!filters.value.hiddenGem ||
    !!filters.value.breakthrough ||
...
const activeFilterCount = computed(() => {
  return (filters.value.topics?.length ?? 0) +
    ...
    (filters.value.estimatedTime?.length ?? 0) +
    (filters.value.category ? 1 : 0) +
    (filters.value.featured ? 1 : 0) +
    (filters.value.hiddenGem ? 1 : 0) +
    (filters.value.breakthrough ? 1 : 0) +
...
<template>
  ...
  >
    ...
    <!-- Special Filters -->
    <div class="space-y-3">
      <h3 class="text-sm font-medium text-gray-900 dark:text-white">Special Collections</h3>
      <div class="flex flex-wrap gap-2">
        <button
          @click="toggleFeatured"
          :class="[
            'px-3 py-1.5 text-sm font-medium rounded-lg border transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent inline-flex items-center gap-1.5',
            filters.featured
              ? 'bg-amber-500 text-white border-amber-500'
              : 'bg-white dark:bg-black text-gray-700 dark:text-gray-300 border-gray-200 dark:border-gray-800 hover:border-amber-500'
          ]"
        ...
        <button
          @click="toggleHiddenGem"
          :class="[
            'px-3 py-1.5 text-sm font-medium rounded-lg border transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent inline-flex items-center gap-1.5',
            filters.hiddenGem
              ? 'bg-purple-500 text-white border-purple-500'
              : 'bg-white dark:bg-black text-gray-700 dark:text-gray-300 border-gray-200 dark:border-gray-800 hover:border-purple-500'
          ]"
          :aria-pressed="filters.hiddenGem"
        >
          <span>💎</span>
          Hidden Gems
        </button>
        <button
          @click="toggleBreakthrough"
          :class="[
            'px-3 py-1.5 text-sm font-medium rounded-lg border transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent inline-flex items-center gap-1.5',
            filters.breakthrough
              ? 'bg-blue-500 text-white border-blue-500'
              : 'bg-white dark:bg-black text-gray-700 dark:text-gray-300 border-gray-200 dark:border-gray-800 hover:border-blue-500'
          ]"
          :aria-pressed="filters.breakthrough"
        >
          <span>💡</span>
          Breakthrough
        </button>
    ...
    <!-- Estimated Time -->
    <div class="space-y-3">
      <h3 class="text-sm font-medium text-gray-900 dark:text-white">Estimated Time</h3>
      <div class="space-y-2">
        <label
        ...
        >
          <input
            type="checkbox"
            :checked="filters.estimatedTime?.includes(time.value)"
            @change="toggleTime(time.value)"
            class="w-4 h-4 rounded border-gray-300 dark:border-gray-700 text-accent focus:ring-2 focus:ring-accent focus:ring-offset-2 dark:focus:ring-offset-black cursor-pointer"
            :aria-label="`Filter by ${time.label}`"
          >
          <svg class="w-4 h-4 text-gray-400 group-hover:text-gray-600 dark:group-hover:text-gray-300" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
